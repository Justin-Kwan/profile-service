...
const CourierService_1 = require("../../../src/domain/user-services/CourierService");
const CourierRepository_1 = require("../../../src/storage/repository/CourierRepository");
const ResponseConstants_1 = require("../../../src/constants/ResponseConstants");
...
const TEST_COURIER_PARAMS_SAME_EMAIL_1 = {
...
const TEST_COURIER_PARAMS_SAME_MOBILE_NUM_1 = {
...
const TEST_COURIER_PARAMS_UPDATED_1 = {
...
const TEST_COURIER_PARAMS_UPDATED_2 = {
...
const courierService = new CourierService_1.CourierService();
...
describe('CourierService Tests', async () => {
    ...
    describe('createUser() tests', async () => {
        it('should create a new courier', async () => {
            ...
            const courierString = await courierService
                .getUser('test_id_1');
            assert_1.strict.deepEqual(courierString, TEST_COURIER_RESPONSE_1);
        });
        it('should create a new courier', async () => {
            // asserting response
            // function under test
            ...
            const courierString = await courierService
                .getUser('test_id_2');
            assert_1.strict.deepEqual(courierString, TEST_COURIER_RESPONSE_2);
        });
        it('should assert that a courier with same id already exists', async () => {
            ...
            await courierService
                .createUser('test_id_2', TEST_COURIER_PARAMS_2);
            // function under test
            const controllerResponse = await courierService.createUser('test_id_2', TEST_COURIER_PARAMS_2);
        ...
        it('should assert that a courier with same email already exists', async () => {
            ...
            await courierService
                .createUser('test_id_1', TEST_COURIER_PARAMS_1);
            // function under test
            const controllerResponse = await courierService.createUser('new_test_id_1', TEST_COURIER_PARAMS_SAME_EMAIL_1);
        ...
        it('should assert that a courier with same mobile number already exists', async () => {
            ...
            await courierService
                .createUser('test_id_1', TEST_COURIER_PARAMS_1);
            // function under test
            const controllerResponse = await courierService.createUser('new_test_id_1', TEST_COURIER_PARAMS_SAME_MOBILE_NUM_1);
    ...
    describe('updateUser() tests', async () => {
        it('should update a single courier with all different fields', async () => {
            // setup
            await courierService.createUser('test_id_1', TEST_COURIER_PARAMS_1);
            // function under test
            const updatedCourier = await courierService.updateUser('test_id_1', TEST_COURIER_PARAMS_UPDATED_1);
            assert_1.strict.deepEqual(updatedCourier, TEST_COURIER_RESPONSE_UPDATED_1);
        });
        it('should update a single courier with all different fields', async () => {
            // setup
            await courierService.createUser('test_id_2', TEST_COURIER_PARAMS_2);
            // function under test
            const updatedCourier = await courierService.updateUser('test_id_2', TEST_COURIER_PARAMS_UPDATED_2);
            assert_1.strict.deepEqual(updatedCourier, TEST_COURIER_RESPONSE_UPDATED_2);
        });
        it('should update a single courier, keeping same email', async () => {
            // setup
            await courierService.createUser('test_id_1', TEST_COURIER_PARAMS_1);
            // function under test
            const updatedCourier = await courierService.updateUser('test_id_1', TEST_COURIER_PARAMS_SAME_EMAIL_1);
            assert_1.strict.deepEqual(updatedCourier, TEST_COURIER_RESPONSE_SAME_EMAIL_1);
        });
        it('should update a single courier, keeping same mobile number', async () => {
            // setup
            await courierService.createUser('test_id_1', TEST_COURIER_PARAMS_1);
            // function under test
            const updatedCourier = await courierService.updateUser('test_id_1', TEST_COURIER_PARAMS_SAME_MOBILE_NUM_1);
            assert_1.strict.deepEqual(updatedCourier, TEST_COURIER_RESPONSE_SAME_MOBILE_NUM_1);
        });
        it('should update a single courier, keeping all same fields', async () => {
            // setup
            await courierService.createUser('test_id_1', TEST_COURIER_PARAMS_1);
            // function under test
            const updatedCourier = await courierService.updateUser('test_id_1', TEST_COURIER_PARAMS_1);
            assert_1.strict.deepEqual(updatedCourier, TEST_COURIER_RESPONSE_1);
        });
        it('should update a single courier with all different fields, without affecting other couriers', async () => {
            ...
            assert_1.strict.deepEqual(unaffectedCourier, TEST_COURIER_RESPONSE_2);
        });
        it('should update a single courier with all different fields, without affecting other couriers', async () => {
            // setup
            await courierService.createUser('test_id_1', TEST_COURIER_PARAMS_1);
            ...
            const unaffectedCourier = await courierService
                .getUser('test_id_1');
            // assert other courier is unaffected
            assert_1.strict.deepEqual(unaffectedCourier, TEST_COURIER_RESPONSE_1);
        });
        it('should assert that resource is not found', async () => {
            // function under test
            const updatedCourier = await courierService.updateUser('non_existent_id', TEST_COURIER_PARAMS_UPDATED_1);
            assert_1.strict.deepEqual(updatedCourier, ResponseConstants_1.RESOURCE_NOT_FOUND);
        });
        it('should assert that resource is not found', async () => {
            // setup
            await courierService.createUser('test_id_1', TEST_COURIER_PARAMS_1);
            // function under test
            const updatedCourier = await courierService.updateUser('test_id_2', TEST_COURIER_PARAMS_UPDATED_1);
            assert_1.strict.deepEqual(updatedCourier, ResponseConstants_1.RESOURCE_NOT_FOUND);
        });
        ...
        it('should assert that a courier with same email already exists', async () => {
            // setup
            await courierService.createUser('test_id_1', TEST_COURIER_PARAMS_1);
            await courierService.createUser('new_test_id_1', TEST_COURIER_PARAMS_UPDATED_1);
            // function under test
            const updatedCourier = await courierService.updateUser('new_test_id_1', TEST_COURIER_PARAMS_SAME_EMAIL_1);
            assert_1.strict.deepEqual(updatedCourier, ResponseConstants_1.RESOURCE_EMAIL_ALREADY_EXISTS);
        });
        ...
        it('should assert that a courier with same email already exists', async () => {
            ...
            await courierService.createUser('new_test_id_1', TEST_COURIER_PARAMS_UPDATED_1);
            // function under test
            const updatedCourier = await courierService.updateUser('new_test_id_1', TEST_COURIER_PARAMS_SAME_MOBILE_NUM_1);
            assert_1.strict.deepEqual(updatedCourier, ResponseConstants_1.RESOURCE_MOBILE_NUM_ALREADY_EXISTS);
        });
    });
    ...
    describe('deleteUser() tests', async () => {
        ...
        it('should delete a single courier', async () => {
            ...
            const doesDeletedCourierExist = await courierRepository
                .existById('test_id_1');
            assert_1.strict.equal(doesDeletedCourierExist, false);
        });
        it('should delete a single courier', async () => {
            // setup
            await courierService
